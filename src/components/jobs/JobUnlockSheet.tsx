...
export default function JobUnlockSheet({
    ...
    if (userId) {
      if (mode === "single" && jobId) {
        await supabase.from("job_unlocks").insert({
          user_id: userId, job_id: jobId, unlock_type: "single",
          amount_paid: amount, currency: "KES",
        });
      }
      if (mode === "pro") {
        await supabase.from("subscriptions").insert({
          user_id: userId, plan: "pro", status: "active",
          billing_cycle: "monthly", amount: 1000, currency: "KES",
        });
      }
      if (savePhone && phone) {
      ...
      const pendingUnlock = { mode, jobId, amount, phone: formatPhone(phone), timestamp: Date.now() };
  ...
  const handleMpesa = async () => {
    ...
    try {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/mpesa-stk-push`, {
        ...
        body: JSON.stringify({
          phone: formatted, amount, packageName: label,
          orderId: generatedOrderId, fullName: "Job Unlock", email: "unlock@cvedge.live",
        }),
      });
  ...
  const handlePayPalSuccess = async () => { await handleSuccess(); };
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-[440px] rounded-t-2xl sm:rounded-2xl border border-border bg-card relative"
        ...
        <div className="max-h-[85vh] overflow-y-auto overscroll-contain p-6">
          ...
          {step === "form" && (
            <div className="space-y-4">
              <div className="text-center">
                <div className={`w-14 h-14 rounded-full mx-auto flex items-center justify-center mb-3 ${themeColor === "blue" ? "bg-blue-500/10" : "bg-amber-500/10"}`}>
                  {mode === "pro"
              ...
              <div className={`rounded-xl border p-4 text-center ${themeColor === "blue" ? "border-blue-500/20 bg-blue-500/5" : "border-amber-500/20 bg-amber-500/5"}`}>
              ...
              <div className="relative z-10">
                ...
                <Input
                  ...
                  autoComplete="tel"
                  autoFocus
                  className="mt-1 text-base h-12 touch-manipulation bg-background relative z-10"
                />
              </div>

              <div className="flex items-center gap-2">
                <Checkbox checked={savePhone} onCheckedChange={(v) => setSavePhone(!!v)} id="save-phone" />
                <label htmlFor="save-phone" className="text-xs text-muted-foreground cursor-pointer select-none">
          ...
          {step === "failed" && (
            <div className="py-4 space-y-5 text-center">
              ...
              <Button variant="outline" onClick={() => setStep("form")} className="w-full">Try Again</Button>
              <PayPalButton amountUsd={amountUsd} description={label} onSuccess={handlePayPalSuccess} />
            </div>
          )}
          ...
          {step === "signup_prompt" && (
            <div className="py-4 space-y-5 text-center">
              ...
              <Button
                onClick={() => { handleClose(); onUnlocked(); window.location.href = "/signup?redirect=/jobs"; }}
              ...
              <Button
                variant="ghost"
                onClick={() => { handleClose(); onUnlocked(); window.location.href = "/login?redirect=/jobs"; }}
