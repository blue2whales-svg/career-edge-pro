...
import {
  X, Lock, Phone, CheckCircle, AlertTriangle, Loader2,
  MessageCircle, Circle, ShieldCheck, Users,
} from "lucide-react";
...
const PACKAGES = [
  ...
  { value: "executive", label: "Executive CV — KES 2,490", amount: 2490 },
  { value: "cover-letter", label: "Cover Letter Only — KES 900", amount: 900 },
  { value: "cv-review", label: "Expert CV Review — KES 900", amount: 900 },
...
function PaybillCard({ orderId, amount }: { orderId: string; amount: number }) {
  return (
    <div className="rounded-xl border border-border bg-muted/30 p-4 text-left space-y-2 text-sm">
      ...
      <div className="flex justify-between"><span className="text-muted-foreground">1. Go to</span><span className="font-medium">M-Pesa → Pay Bill</span></div>
      <div className="flex justify-between"><span className="text-muted-foreground">2. Business No:</span><span className="font-mono font-bold text-primary">{PAYBILL}</span></div>
      <div className="flex justify-between"><span className="text-muted-foreground">3. Account No:</span><span className="font-mono font-bold text-xs">{orderId}</span></div>
      <div className="flex justify-between"><span className="text-muted-foreground">4. Amount:</span><span className="font-mono font-bold">KES {amount.toLocaleString()}</span></div>
    </div>
  );
}

// ✅ UNLOCK HANDLER — invisible component, fires localStorage unlock on payment success
function PaymentSuccessHandler({
  paymentStatus, selectedPackage, amount, onPaymentSuccess,
}: {
  paymentStatus?: any; selectedPackage?: string; amount?: number; onPaymentSuccess?: () => void;
}) {
...
export default function MpesaPaymentModal({ open, onClose, defaultPackage = "professional", onPaymentSuccess }: MpesaPaymentModalProps) {
  ...
  const [paymentStatus, setPaymentStatus] = useState<any>(null); // ✅ drives PaymentSuccessHandler
  ...
  useEffect(() => {
    if (open) {
      setStep("form"); setLoading(false); setSelectedPackage(defaultPackage);
      setTimelineStage("initiated"); setRetryCountdown(0); setRetryAttempt(0); setPaymentStatus(null);
    }
    return () => {
  ...
  const startPolling = (crId: string) => {
    pollRef.current = setInterval(async () => {
      try {
        const { data } = await supabase.from("orders").select("status, mpesa_receipt").eq("mpesa_checkout_request_id", crId).maybeSingle();
        ...
        if (row?.status === "paid") {
          ...
          setMpesaCode(row.mpesa_receipt || ""); setConfirmedAmount(pkg.amount);
          setTimelineStage("confirmed"); setStep("success");
          setPaymentStatus({ status: "paid", success: true }); // ✅ triggers unlock
          trackPurchase(confirmedAmount || pkg.amount, "KES");
          onPaymentSuccess?.();
        } else if (row?.status === "failed" || row?.status === "payment_failed") {
          clearInterval(pollRef.current!); clearTimeout(timeoutRef.current!);
          setTimelineStage("failed"); setStep("failed");
        }
  ...
  const fireStkPush = async (formattedPhone: string, generatedOrderId: string) => {
    ...
    const res = await fetch(`${supabaseUrl}/functions/v1/mpesa-stk-push`, {
      ...
      body: JSON.stringify({ phone: formattedPhone, amount: pkg.amount, packageName: pkg.label, orderId: generatedOrderId, fullName: fullName.trim(), email: email.trim() }),
    });
  ...
  const startRetryCountdown = (formattedPhone: string, generatedOrderId: string, attempt: number) => {
    ...
    countdownRef.current = setInterval(() => {
      seconds -= 1; setRetryCountdown(seconds);
      if (seconds <= 0) { if (countdownRef.current) clearInterval(countdownRef.current); attemptStkPush(formattedPhone, generatedOrderId, attempt); }
    }, 1000);
  };
  ...
  const attemptStkPush = async (formattedPhone: string, generatedOrderId: string, attempt: number) => {
    try {
      ...
      if (data?.ResponseCode === "0") {
        setCheckoutRequestId(data.CheckoutRequestID || ""); setTimelineStage("prompt_sent");
        setRetryCountdown(0); setRetryAttempt(0);
      ...
      if (isBusyError(data) && attempt < maxRetries) {
        ...
        startRetryCountdown(formattedPhone, generatedOrderId, attempt + 1); return;
      }
      console.error("STK push failed:", data); setTimelineStage("failed"); setStep("failed"); setRetryCountdown(0);
      ...
      setTimelineStage("failed"); setStep("failed"); setRetryCountdown(0);
  ...
  const handleSubmit = async () => {
    ...
    if (formattedPhone.length < 12) { toast.error("Please enter a valid Kenyan phone number"); return; }
    setLoading(true); setRetryAttempt(0); setRetryCountdown(0);
    const generatedOrderId = "CVE-" + Date.now();
    setOrderId(generatedOrderId); setStep("waiting"); setTimelineStage("initiated");
    await attemptStkPush(formattedPhone, generatedOrderId, 1);
    setLoading(false);
  };

  // ✅ shared handler for both "I Paid via Paybill" buttons (waiting + failed steps)
  const handleManualConfirm = async () => {
    ...
    if (data?.status === "paid") {
      ...
      setPaymentStatus({ status: "paid", success: true }); // ✅ triggers unlock
      trackPurchase(confirmedAmount || pkg.amount, "KES");
      onPaymentSuccess?.(); toast.success("Payment confirmed! 🎉");
    } else {
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)" }} onClick={handleClose}>
          <motion.div
            initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 10 }} transition={{ type: "spring", stiffness: 300, damping: 28 }}
              ...
              <div className="py-4 space-y-5">
                ...
                <div className="border-t border-border pt-4 mt-4">
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    <Button variant="outline" onClick={handleManualConfirm} className="w-full text-sm gap-1.5">
                      <CheckCircle className="h-4 w-4" /> I Paid via Paybill
                    </Button>
                    <a href={`https://wa.me/254700000000?text=Hi%20CV Edge!%20I%20need%20help%20with%20payment.%20Order:%20${orderId}`} target="_blank" rel="noopener noreferrer">
            ...
            {step === "success" && (
              <div className="py-4 space-y-5">
                <div className="text-center">
                  ...
                  <p className="text-sm text-primary font-semibold">✅ KES {(confirmedAmount || pkg.amount).toLocaleString()} received</p>
                  {mpesaCode && (<p className="text-xs text-muted-foreground mt-1">M-Pesa Code: <strong>{mpesaCode}</strong></p>)}
                  <p className="text-sm text-muted-foreground mt-2">Welcome, {fullName.split(" ")[0]}!</p>
                </div>
                <div className="rounded-xl border border-border bg-muted/20 p-4"><PaymentTimeline stage="confirmed" /></div>
                <Button onClick={() => { handleClose(); window.location.href = selectedPackage.startsWith("recruiter-") ? `/post-job?pkg=${selectedPackage}` : "/cv-builder"; }}
            ...
            {step === "failed" && (
              <div className="py-4 space-y-5">
                ...
                <div className="rounded-xl border border-border bg-muted/20 p-4"><PaymentTimeline stage="failed" /></div>
                <PaybillCard orderId={orderId} amount={pkg.amount} />
                <div className="flex flex-col gap-2">
                  <Button variant="outline" onClick={handleManualConfirm} className="w-full text-sm gap-1.5">
                    <CheckCircle className="h-4 w-4" /> I've Paid via Paybill
                  </Button>
                  <Button variant="outline" onClick={() => { setStep("form"); setTimelineStage("initiated"); }} className="w-full text-sm">
                    Try Again
                  </Button>
                  <a href={`https://wa.me/254700000000?text=Hi%20CV Edge!%20I%20need%20help%20with%20payment.%20Order:%20${orderId}`} target="_blank" rel="noopener noreferrer">
                    <Button variant="ghost" className="w-full text-sm gap-1.5 text-primary hover:text-primary/80">
            ...
            {/* ✅ UNLOCK HANDLER — invisible, fires when paymentStatus is set */}
