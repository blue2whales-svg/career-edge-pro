...
import { Shield, Trash2, Star, CheckCircle2, RefreshCw, Search, Eye, Flame } from "lucide-react";
      ...
      <section className="relative z-10 pt-16 sm:pt-24 pb-10 px-4">
        <div className="container max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h1 className="text-3xl font-serif font-bold">Job Control Panel</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {[
              ...
              { label: "Hot Jobs", value: jobs.filter((j) => j.hot).length, color: "text-brand-red" },
              { label: "Featured", value: jobs.filter((j) => j.featured).length, color: "text-yellow-500" },
                    ...
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        <span className={`text-xs font-mono font-bold ${
                          (job.hot_score || 0) >= 70 ? "text-brand-red" : (job.hot_score || 0) >= 40 ? "text-yellow-500" : "text-muted-foreground"
                        }`}>
                          {job.hot_score || 0}
                        </span>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          {job.hot && <Flame className="h-3 w-3 text-brand-red" />}
                          {job.featured && <Star className="h-3 w-3 text-yellow-500" />}
          ...
          {!isLoading && filtered.length > 100 && (
            <p className="text-xs text-muted-foreground text-center mt-4">
              Showing 100 of {filtered.length} jobs
            </p>
          )}
        </div>
      </section>
