    ...
    <div style={{ background: "#fff", fontFamily: "Georgia, serif", fontSize: 8, minHeight: 700 }}>
      <div style={{ background: "#0f172a", padding: "20px 28px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ width: 50, height: 50, borderRadius: "50%", background: "#c9a84c", display: "flex", alignItems: "center", justifyContent: "center", color: "#0f172a", fontSize: 16, fontWeight: "bold" }}>{cv.name.split(" ").map(n => n[0]).join("").slice(0,2)}</div>
        </div>
      </div>
...
export default function CVEditorPage() {
  ...
  return (
    <PageLayout>
      <section className="relative z-10 pt-6 pb-20 px-4">
        <div className="container max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-6">
            <button onClick={() => navigate("/cv-builder")} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6 max-h-[85vh] overflow-y-auto pr-2">
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2"><Input placeholder="Full Name" value={cv.name} onChange={e => update("name", e.target.value)} /></div>
                  <div className="col-span-2"><Input placeholder="Professional Title" value={cv.title} onChange={e => update("title", e.target.value)} /></div>
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={addExp} className="flex items-center gap-1 text-xs text-primary hover:underline"><Plus className="h-3 w-3" /> Add</button>
                </div>
                  ...
                  {cv.experiences.map((exp, i) => (
                    <div key={i} className="border border-border rounded-lg p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        {cv.experiences.length > 1 && <button onClick={() => removeExp(i)} className="text-destructive hover:opacity-70"><Trash2 className="h-3.5 w-3.5" /></button>}
                      </div>
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={addEdu} className="flex items-center gap-1 text-xs text-primary hover:underline"><Plus className="h-3 w-3" /> Add</button>
                </div>
                  ...
                  {cv.educations.map((edu, i) => (
                    <div key={i} className="border border-border rounded-lg p-4 space-y-2">
                      <div className="flex justify-between">
                        ...
                        {cv.educations.length > 1 && <button onClick={() => removeEdu(i)} className="text-destructive hover:opacity-70"><Trash2 className="h-3.5 w-3.5" /></button>}
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="col-span-2"><Input placeholder="Degree / Qualification" value={edu.degree} onChange={e => updateEdu(i, "degree", e.target.value)} /></div>
                        ...
                        <div className="col-span-2"><Input placeholder="Grade / Honours (optional)" value={edu.grade} onChange={e => updateEdu(i, "grade", e.target.value)} /></div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="sticky top-24">
              ...
              <div className="rounded-xl border border-border overflow-hidden shadow-lg" style={{ maxHeight: "80vh", overflowY: "auto" }}>
                {preview(cv)}
              </div>
            </div>
          </div>
